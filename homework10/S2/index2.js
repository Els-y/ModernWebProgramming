(function() {
  ...
  p.robotBtnClick = function() {
    if (this.$dom.hasClass("hasnum")) {
      if (this.$dom.next().length !== 0)
        this.$dom.next().get(0)._btn.robotBtnClick();
      return;
    }
    this.btnLoading();
    this.robotAjaxGet();
  ...
  p.robotAjaxGet = function() {
    ...
    this.xhr = $.get("/" + new Date().getTime(), null, function(data, textStatus) {
      ...
      $notHasnum.removeClass("button-disabled")
                .find(".tag").removeClass("tag-disabled");
      if ($notHasnum.length === 0) $("#info-bar").removeClass("answer-disabled").click();
      if (that.$dom.next().length !== 0) that.$dom.next().get(0)._btn.robotBtnClick();
    });
  }
  ...
  p.apbClickHandler = function() {
    if ($(".waiting").length !== 0) return;
    $(".button").get(0)._btn.robotBtnClick();
  }
